![.NET Core](https://github.com/MyJetEducation/Template.Library/workflows/.NET%20Core/badge.svg)
...
where ${path} is the **full** path to the clonned directory (where the folder .template.config placed) **without trailing slash** (e.g. `D:\Root\Template.Library\template`)

### Create

`
dotnet new myjeteducationlibrary -n {ServiceName} -o {ServiceDirectory}
`

e.g.

`
dotnet new myjeteducationlibrary -n SomeService -o SomeServiceFoulder
`
